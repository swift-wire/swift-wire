...
package func renderWireGraph(
    ...
    appendStruct(
        ...
        roots: declaredRoots(
            in: bindingsByIdentity(topologicalOrder),
            multibindingKeys: multibindingKeys,
            conformances: graphConformances,
    ...
    resolveStoragePatches(storagePatches, in: &lines)
...
private func appendNamedGraphs(
    ...
    patches: inout [GraphStoragePatch]
) {
    ...
    for (name, order) in named.sorted(by: { $0.key < $1.key }) {
        ...
        appendStruct(
            ...
            // Conformances are emitted onto the default graph and its *variants*, never onto a
            // `@Container`'s — `appendAllGraphConformances` puts them there — so only a variant's root
            // set carries them. Mirrors `pruningPolicy`'s `containerKey == nil` test.
            roots: declaredRoots(
                in: bindingsByIdentity(order),
                multibindingKeys: multibindingKeys,
                conformances: variant[name] != nil ? graphConformances : [],
                externalModules: externalModules
            ),
            into: &lines,
            entries: &entries,
...
private func appendStruct(
    ...
    existentialPromotions: [ExistentialPromotion] = [],
    graphInputsType: String? = nil,
    // The M7b root model for *this* graph — what it must construct even though nothing in it consumes
    // them. M7c.1 makes it also the core of what the graph *stores*; see `Retention.swift`.
    ...
    let propertyBlockIndex = lines.count
    lines.append(storagePlaceholder)
    ...
    let returnLineIndex = lines.count
    lines.append(storagePlaceholder)
