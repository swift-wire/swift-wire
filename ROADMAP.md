...
- **M4: lifecycle orchestration — complete.** The forcing case: task-cluster moved onto a real (Soto) DynamoDB client vs the in-memory one, needing orderly shutdown. The `@Teardown` annotation existed from M1 (recognised and recorded, but inert); M4 emits the **app-scope** teardown walk — `teardown()` on the generated graph, calling each `@Teardown` action in reverse dependency order, run at shutdown via WireHummingbird's `teardownService` (a `ServiceLifecycle` service prepended so it shuts down last, after the server stops). Teardown-action failures are collected and logged so one doesn't stop the rest. **Request-/job-scope teardown** needs request scope and is **M5**. **Init-failure partial teardown** — tearing down already-constructed bindings in reverse before a bootstrap rethrow — was deferred to **M7c** and **shipped there as M7c.5 (2026-09)**: its implementation is fixed by the construction scheduler that pass settles, so it landed once rather than being rewritten. The deferral's reasoning held and pointed one step further than it expected — after M7c.4 the constructed set is three things at once, so it is accumulated as each binding is built rather than inspected on failure. Scope entry did not get it and the gap there is latent, recorded as [#339](https://github.com/tachyonics/swift-wire/issues/339). The forcing case moved task-cluster onto the Soto AWS stack (`AWSClient` `@Provides @Teardown`), validated against a real table via a LocalStack integration test. See [TeardownDesign.md](Documentation/Notes/TeardownDesign.md).
...
- **The router path's remaining allocation group** — path-parameter values, collected positionally and
  ...
  [tachyonics/wire-mvc#175](https://github.com/tachyonics/wire-mvc/issues/175): `[String: Substring]` is written into
...
**M7's reachability pass and its construction scheduling have since shipped**; what remains of it (M7a, M7e) waits on triggers rather than on any of these.
...
- **App-scope teardown does not run under `@WireMVCBootstrap`.** Surfaced while writing the README's
  ...
  [tachyonics/wire-mvc#177](https://github.com/tachyonics/wire-mvc/issues/177). **Request-scope teardown
