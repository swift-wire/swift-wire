...
M0 through M5 are complete, as is M6a (testing). The historical implementation plans are archived at
...
Within **M6 (surface completeness)**, **M6a (testing) is complete** (below); **M6b** is next. The detail sections after
...
- **M6: surface completeness / DX.** The remaining pre-1.0 *surface* work — features that make idiomatic apps expressible and unblock the last examples, ahead of M7's invisible perf passes. The principle is *complete the surface before optimizing it*. Ordered so foundational, example-unblocking work leads:
  ...
  - **M6d — advanced OpenAPI integration.** Bring WireMVC's request-scope + typed-param/response DX (`@JSONResponse`/`@JSONBody`-style ergonomics) onto OpenAPI operations, beyond today's spec-driven-handlers-only surface. Genuinely new (not a prior deferral, so it needs a design pass before build); the largest of the four and sequenced last. Design pass: [WireOpenAPIAdvanced.md](Documentation/Notes/WireOpenAPIAdvanced.md).
