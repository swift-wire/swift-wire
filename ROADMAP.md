...
M0 through M5 are complete, as are M6a (testing) and M6b (the request-logger seam). The historical implementation plans are archived at
...
- **M6: surface completeness / DX.** The remaining pre-1.0 *surface* work — features that make idiomatic apps expressible and unblock the last examples, ahead of M7's invisible perf passes. The principle is *complete the surface before optimizing it*. Ordered so foundational, example-unblocking work leads:
  ...
  - **M6c — `@Configuration` / WireConfiguration.** The swift-configuration adapter: `@Configuration(forKey:default:)` at `@Inject`/`@Provides` parameter sites, desugared to a synthesized `ConfigReader` binding — sugar over the graph machinery, riding the shared "adapter replaces the binding" primitive (already built for M5.4 request scope). **M6b moved the ground under this in two ways.** The old justification — *every example hand-rolls config* — is spent: all three wire-mvc-examples runtimes now build **one** `ConfigReader` before the graph and pass it in as a `@GraphInputs` value, so nothing hand-rolls and no provider constructs its own reader. What is left is the narrower, still-real win: `@Configuration` makes the *value* the binding rather than the reader, so a consumer depends on `Int`/`String` instead of on a collaborator it has to call, the key is visible in the signature, and a test substitutes a value rather than a configured reader. And the migration target is now concrete and in-tree rather than a single task-cluster line — every example has providers of exactly the shape it removes (`config.string(forKey: "couchdb.host", default: "localhost")` inside the body becomes an annotated parameter), which is the validation gate. Expected to need **no swift-wire change**: it is a plugin-level source transformation producing existing constructs, and M6b's `@Bind` multibinding overloads closed the one parameter-site gap. Full design in the [WireConfiguration preview](#wireconfiguration-scheduled-as-m6c) below.
