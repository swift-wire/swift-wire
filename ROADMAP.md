...
- **M6: surface completeness / DX.** The remaining pre-1.0 *surface* work — features that make idiomatic apps expressible and unblock the last examples, ahead of M7's invisible perf passes. The principle is *complete the surface before optimizing it*. Ordered so foundational, example-unblocking work leads. **The examples this milestone exists to unblock are enumerated outside it** — see [RemainingSurfaceWork.md](Documentation/Notes/RemainingSurfaceWork.md), which sequences the parity and streaming tracks together with wire-mvc's router backlog, and records the open question of whether those become an M6e or an explicitly post-M6 track:
