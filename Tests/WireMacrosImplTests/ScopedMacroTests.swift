...
final class ScopedMacroTests: XCTestCase {
    ...
    // MARK: - Scope block (on a namespace enum)

    /// `@Scoped(seed:)` on a namespace enum is a scope-block marker — the
    /// member role synthesises nothing (the plugin does the routing), and
    /// it is not an error.
    func test_scopedOnNamespaceEnum_emitsNothingAsScopeBlock() {
        assertMacroExpansion(
            """
            @Scoped(seed: RequestSeed.self)
            enum RequestProviders {
                static let tag: Tag = Tag()
            }
            """,
            expandedSource: """
                enum RequestProviders {
    ...
    func test_scopedOnProtocol_throwsUnsupportedDeclarationError() {
        assertMacroExpansion(
            ...
            diagnostics: [
                DiagnosticSpec(
                    message:
                        "@Scoped can only be applied to a struct, class, or actor, or a namespace enum (as a scope block).",
                    line: 1,
                    column: 1,
                    severity: .error
                )
